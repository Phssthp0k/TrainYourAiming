...
class FourSquares_Class extends Game_Class
{
	...
	OnClick(mouseEvents)
	{
		...
		if( this.quadratoNW.IsHit(mousePos) > 0) { log("NW ["+mousePos.x+"]["+mousePos.y+"]"); this.CM.overlay_Objects_List.Add(new CrossAir_Class("HitNW", 'white', mousePos)); }
		if( this.quadratoSW.IsHit(mousePos) > 0) { log("SW ["+mousePos.x+"]["+mousePos.y+"]"); this.CM.overlay_Objects_List.Add(new CrossAir_Class("HitSW", 'blue', mousePos)); }
		if( this.quadratoNE.IsHit(mousePos) > 0) { log("NE ["+mousePos.x+"]["+mousePos.y+"]"); this.CM.overlay_Objects_List.Add(new CrossAir_Class("HitNE", 'green', mousePos)); }
		if( this.quadratoSE.IsHit(mousePos) > 0) { log("SE ["+mousePos.x+"]["+mousePos.y+"]"); this.CM.overlay_Objects_List.Add(new CrossAir_Class("HitSE", 'red', mousePos)); }
	}
}